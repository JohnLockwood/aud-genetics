...
* [x] Complete the [missingness checkpoint](lessons/01-genomics-data-and-qc/README.md#second-checkpoint-missingness): predict sample and variant missingness by hand, generate PLINK's reports, and reconcile the results. See the [review](notes/2026-09-10-phase-1-learning-bridge.md#follow-up-missingness-checkpoint-reviewed).
* [x] Complete the [missingness-filtering checkpoint](lessons/01-genomics-data-and-qc/README.md#third-checkpoint-filtering-on-missingness): predict and compare separate `--geno 0.20` and `--mind 0.20` runs, including the behavior of values exactly on the threshold. See the [checkpoint review](notes/2026-09-11-missingness-filtering.md).
* [x] Complete the [alternate-allele-frequency checkpoint](lessons/01-genomics-data-and-qc/README.md#fourth-checkpoint-alternate-allele-counts-and-frequencies): calculate counts and frequencies by hand, distinguish alternate from minor allele, and compare the results with PLINK's `.acount` and `.afreq` reports. See the [checkpoint review](notes/2026-09-11-allele-frequency.md).
* [x] Complete the [genetics-foundations bridge](lessons/01-genomics-data-and-qc/genetics-foundations.md): connect chromosomes, loci, alleles, and genotypes, and explain the assumptions conveyed by “autosomal, diploid, biallelic.” See the [checkpoint review](notes/2026-09-15-snp-allele-terminology.md#follow-up-foundations-checkpoint-reviewed).
* [x] Complete the [sample-MAF and `--maf` checkpoint](lessons/01-genomics-data-and-qc/README.md#fifth-checkpoint-sample-maf-and-plink-filtering): derive the sample minor-allele frequency for each synthetic variant, then predict and verify PLINK's boundary behavior. See the [checkpoint review](notes/2026-09-15-minor-allele-frequency-lesson.md#follow-up-checkpoint-reviewed).
* [x] Complete the [observed-genotype-frequency and HWE checkpoint](lessons/01-genomics-data-and-qc/README.md#sixth-checkpoint-observed-genotype-frequencies-and-hardy-weinberg-equilibrium): derive observed and expected genotype frequencies, compare them with PLINK's reports, and explain what an HWE test can and cannot establish. See the [checkpoint review](notes/2026-09-16-hwe-checkpoint.md).
* [x] Complete the [per-sample heterozygosity checkpoint](lessons/01-genomics-data-and-qc/README.md#seventh-checkpoint-per-sample-heterozygosity): calculate observed homozygous and heterozygous calls, interpret PLINK's `F` estimate, and distinguish sample-level `--het` from per-variant HWE testing. See the [checkpoint review](notes/2026-09-17-heterozygosity-lesson.md#follow-up-checkpoint-reviewed).
* [ ] Assemble the Phase 1 capstone: document an ordered QC run and sample/variant attrition table, explaining which thresholds are teaching demonstrations and what would be required to justify them in research.
